...
to setup
  ...
  create-turtles number [
    ...
    if (personality = "coward") [set color blue]
    if (personality = "hero") [set color red]
    if (personality = "mixed") [set color one-of [red blue]]
...
;    create-link-with enemy [set color black]
  ]
  reset-ticks
end
...
to act-cowardly
  ;; move as far away from the friend, as the friend is from the enemy
  facexy [xcor] of friend + ([xcor] of friend - [ycor] of enemy) / 2
...
to act-bravely
  ;; move toward midpoint between friend and enemy
